...
class AMSGrad(optimizer.Optimizer):
    def __init__(self, learning_rate=0.0001, beta1=0.9, beta2=0.99, epsilon=1e-8, use_locking=False, name="AMSGrad"):
    ...
    def _create_slots(self, var_list):
        ...
        for v in var_list :
            ...
            lr_v = np.random.choice([10.0, 1.0, 0.1, 0.001], v.get_shape())
            lr_v = tf.constant(lr_v, dtype=v.dtype.base_dtype, shape=v.get_shape())
            self._get_or_make_slot(v, lr_v, "lr_v", self._name)
    ...
    def _resource_apply_dense(self, grad, var):
        ...
        lr_v = self.get_slot(var, "lr_v")
        lr = lr_v*(lr_t * math_ops.sqrt(1 - beta2_power) / (1 - beta1_power))
